export const getProjectData = () => {
	return [
		{
			name: 'ConnectME',
			about: 'I wanted to create an application that streamlined a few functionalities that Facebook already provided, but in a rather user-unfriendly way. The goal of ConnectME was to create a platform to search friends in a particular location and then message one particular friend or a group of friends in that location. A user should be able to login and logout, send and store messages to and from friends, and eventually, store groups of friends in particular locations. In this project, I implemented Firebase OAuth and database, used the Facebook Graph API, and tested with Enzyme/Jest. This was a 2-week project.',
		...
		{
			name: 'MovieTracker',
			about: 'Everyone wants all the new info about movies. MovieTracker gives you this and more. The goal of MovieTracker was to create a web application that allows users to look at information for newly released movies, create an account to save favorites, and login or logout from that account. This project is working off the The Movie DB API (https://www.themoviedb.org/documentation/api). User interface design came with challenges such as what to do if a user was not logged in and tried to add a favorite? This was a week and a half long project.',
